...
\usepackage{tabularx} % Pro řízení šířky tabulky
\usepackage{booktabs} % Pro lepší horizontální linky
\usepackage{multirow}
\usepackage{ragged2e}
%\usepackage{natbib} 
\usepackage{graphicx}
\usepackage{float}
\usepackage{listings}
\usepackage{xcolor} % Pro barvičky

\lstset{
    basicstyle=\ttfamily\small, % Neproporcionální písmo
    breaklines=true,            % Automatické zalamování řádků
    frame=single,               % Rámeček kolem kódu
    numbers=left,               % Číslování řádků vlevo
    keywordstyle=\color{blue},  % Barva klíčových slov
    commentstyle=\color{green}, % Barva komentářů
    stringstyle=\color{red}     % Barva textových řetězců
}
...
\makeatother
\setcounter{secnumdepth}{3}  % číslovat až do úrovně subsubsection
...
\newcommand{\nazevcz}{\ZT{Analýza spotřeby energie s využitím metod strojového učení}}        % zde VYPLŇTE český název práce (přesně podle zadání!)
\newcommand{\nazeven}{\ZT{Energy Consumption Analysis Using Machine Learning Methods}}     % zde VYPLŇTE anglický název práce (přesně podle zadání!)
\newcommand{\autor}{\ZT{Ondřej Thomas}}           % zde VYPLŇTE své jméno a příjmení
\newcommand{\rok}{\the\year}                
\newcommand{\vedouci}{\ZT{Kubera Petr, RNDr. Ph.D.}}         
% zde VYPLŇTE jméno a příjmení vedoucího práce, včetně titulů
\newcommand{\vedouciDAT}{\ZT{Petru Kuberovi, Ph.D.}}   
